...
app.config['MAIL_USERNAME'] = '13e4f03f479e91'
app.config['MAIL_PASSWORD'] = 'df82053c0a26e2'
...
def parameters():
    ...
    if age < 18:
        return jsonify(message=f" Sorry {name} not old enough."), 401
    else:
        return jsonify(message= f"congrats {name} you are old enough.")
...
def retrieve_password(email: str):
    ...
    if user:
        msg = Message("Your Planetary API password is " + user.password,
                      sender="admin@planetary-api.com", recipients= [user.email])
        mail.send(msg)
